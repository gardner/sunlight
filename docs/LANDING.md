...
collection matters, and how authorities can recognize legitimate Sunlight Requests.

It is separate from:

* `https://admin.sunlight.nz`: internal operations app
* `https://requests.sunlight.nz`: authority response app
...
* primary navigation
* clear path for authority staff who received a request
...
* how authorities can respond
* how to contact Sunlight

Calls to action:

* Authorities: respond using the unique link or reply address in the email they
  received.
...
This page should help authority staff verify and respond to a SunlightRequest.

It should explain:

* Sunlight sends recurring requests asking for OIA/LGOIMA requests and responses
  received by the authority for a defined period.
* Each request includes a unique reply email address.
* Each request includes a unique upload URL for large files.
* Authorities do not need to create an account.
* Authorities should not use the public landing page to upload files.
* If an authority is unsure whether a request is legitimate, it can contact
...
* recurring requests to selected authorities
...
* Sunlight's own request to authorities: `SunlightRequest`
* OIA/LGOIMA request material disclosed by authorities: disclosed request/response
  material
...
* authority response/support email
...
* Sunlight receives and stores material from authorities.
...
* Authorities can contact Sunlight about mistaken or sensitive submissions.
...
* `https://requests.sunlight.nz` only when explaining authority response links
...
* tokenized authority response URLs

## Non-Goals

The landing app does not:

* collect authority files
* authenticate admins
