...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--brave-results", type=int, default=8, help="Maximum Brave results per agency.")
...
def scrape_agency(
    ...
    *,
    ...
    seed_urls = brave_search_urls(agency, session=session, api_key=brave_api_key, limit=brave_results, timeout=timeout)
    pages = fetch_agency_pages(agency, session=session, timeout=timeout, max_pages=max_pages, seed_urls=seed_urls)
...
def fetch_agency_pages(
    ...
    *,
    ...
    pages = [(homepage, agency.home_page_url, "homepage")] if homepage else []
    visited = {agency.home_page_url} if homepage else set()
    seeds = [PageLink(url=url, text="Brave Search result") for url in seed_urls or []]
    fallback_links = find_candidate_links(homepage, agency.home_page_url, max_links=max_pages * 20) if homepage else []
    ...
    while pending and len(pages) < max_pages:
        ...
        print(f"{agency.name}: fetching {len(pages) + 1}/{max_pages} {link.url}", flush=True)
...
def brave_search_urls(
    agency: Agency, *, session: requests.Session, api_key: str | None, limit: int, timeout: float
) -> list[str]:
    ...
    try:
        ...
        print(f"Brave search failed for {agency.name}: {error}", flush=True)
        return []
    ...
    for result in results:
        ...
        if url and should_follow_link(url, text, agency.home_page_url):
            urls.append(url)
    print(f"{agency.name}: seeded {len(urls)} Brave result URLs", flush=True)
