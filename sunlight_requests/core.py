...
class RequestStore:
    ...
    def preview_cycle(self, cycle_id: int) -> list[dict[str, Any]]:
        ...
        preview = []
        for authority in self._eligible_authorities():
            preview.append(
                {
                    "cycle_id": cycle_id,
                    "month": cycle["month"],
                    ...
                    "legal_regime": authority["legal_regime"],
                    "request_email": authority["request_email"],
    ...
    def approve_cycle(
        ...
        *,
        ...
        excluded_authority_ids: set[int] | None = None,
    ) -> list[RequestCase]:
        excluded_authority_ids = excluded_authority_ids or set()
        try:
            cycle = self._cycle(cycle_id)
            authorities = [
                authority
                for authority in self._eligible_authorities()
                if authority["id"] not in excluded_authority_ids
            ]
            now = utcnow()
            for authority in authorities:
                case = self._get_case_for_authority_cycle(authority["id"], cycle_id)
                if case is None:
                    if authority["template_id"] is None:
                        raise ValueError(f"Authority {authority['name']} has no template")
                    ...
                    self._audit(
                        ...
                        actor_type="operator",
                        actor_id=actor_id,
                        metadata={"authority_id": authority["id"], "cycle_id": cycle_id},
                    )
            ...
            self._audit(
                ...
                actor_type="operator",
                actor_id=actor_id,
                metadata={"case_count": len(authorities)},
            )
            self.db.commit()
        except Exception:
            self.db.rollback()
    ...
    def _create_case(
        ...
        *,
        ...
        for _ in range(5):
            reply_alias = self._reply_alias(authority_id=authority_id, cycle_month=cycle_month)
            try:
                cursor = self.db.execute(
                    """
                    INSERT INTO request_cases
                        (
                            authority_id, cycle_id, template_id, reply_alias, status,
    ...
    def _queue_outbound_message(
        ...
        *,
        ...
        to_addresses = [authority["request_email"], *json.loads(authority["secondary_emails"])]
        now = utcnow()
        cursor = self.db.execute(
            """
    ...
    def _reply_alias(self, *, authority_id: int, cycle_month: str) -> str:
        authority = self._authority(authority_id)
        token = secrets.token_hex(6)
        return f"requests+{cycle_month}-{authority['slug']}-{token}@{self.alias_domain}"

    def _eligible_authorities(self) -> list[sqlite3.Row]:
    ...
    def _authority(self, authority_id: int) -> sqlite3.Row:
        row = self.db.execute("SELECT * FROM authorities WHERE id = ?", (authority_id,)).fetchone()
        if row is None:
    ...
    def _get_case_for_authority_cycle(self, authority_id: int, cycle_id: int) -> RequestCase | None:
        row = self.db.execute(
            """
            SELECT *
            FROM request_cases
            WHERE authority_id = ? AND cycle_id = ?
            """,
            (authority_id, cycle_id),
        ).fetchone()
